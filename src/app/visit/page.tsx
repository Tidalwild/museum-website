...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return { title: dict.placeholder.visitTitle };
}

export default async function Page() {
  const { locale, dict } = await getTranslation();
  return (
    <PageShell locale={locale} dict={dict} currentPath="/visit">
      <ComingSoon title={dict.placeholder.visitTitle} body={dict.placeholder.visitBody} dict={dict} />
    </PageShell>
  );
}
